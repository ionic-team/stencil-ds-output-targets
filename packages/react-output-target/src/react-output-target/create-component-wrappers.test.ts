...
export const MyComponent = /*@__PURE__*/ createComponent<MyComponentElement, MyComponentEvents>({
  ...
  it('should rewrite event types that conflict with the component name', async () => {
    /**
     * The component implementation may have a type reference whose name matches the component const name
     * that is generated by the createComponentWrappers function.
     *
     * In this case, we need to ensure that the type reference is using the "MyComponentElement" type alias.
     */
    const project = new Project({ useInMemoryFileSystem: true });
    const sourceFiles = await createComponentWrappers({
      components: [
...
type MyComponentEvents = { onMyEvent: EventName<CustomEvent<{ element: HTMLMyComponentElement; context: MyComponentElement; event: Event; }>> };
