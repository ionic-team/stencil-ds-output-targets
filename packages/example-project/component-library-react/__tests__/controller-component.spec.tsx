...
describe('createControllerComponent - events', () => {
  const {
    ...
    augmentController,
  } = createControllerUtils('ion-loading');
  type LoadingOptions = Components.IonLoadingAttributes;
  const IonLoading = createControllerComponent<
    LoadingOptions,
  ...
  test('should create controller component outside of the react component', async () => {
    const { container, baseElement } = render(
      <>
        <IonLoading isOpen={false} onDidDismiss={jest.fn()} duration={2000}></IonLoading>
  ...
  test('should create component and attach props on opening', async () => {
    ...
    const { baseElement, container, rerender } = render(
      <IonLoading isOpen={false} onDidDismiss={onDidDismiss} duration={2000}>
        ButtonNameA
      </IonLoading>,
    );
    ...
    rerender(
      <IonLoading isOpen={true} onDidDismiss={onDidDismiss} duration={2000}>
        ButtonNameA
      </IonLoading>,
    );
  ...
  test('should dismiss component on hiding', async () => {
    const { container, baseElement, rerender } = render(
      <IonLoading isOpen={false} onDidDismiss={jest.fn()} duration={2000}>
        ButtonNameA
      </IonLoading>,
    );
    ...
    rerender(
      <IonLoading isOpen={true} onDidDismiss={jest.fn()} duration={2000}>
        ButtonNameA
      </IonLoading>,
    );
