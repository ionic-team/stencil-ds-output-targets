import { dashToPascalCase, normalizePath } from '@stencil/wrappers-utils';
...
export const createComponentDefinition =
      ...
      Object.entries(output.complexType.references).forEach(([reference, refObject]) => {
        ...
        if (refObject.location === 'local' || refObject.location === 'import') {
          ...
          if (componentCorePackage !== undefined) {
            const dirPath = includeImportCustomElements
              ? `/${customElementsDir || 'components'}`
              : '';
            importLocation = `${normalizePath(componentCorePackage)}${dirPath}`;
          }
          outputsInterface.add(
            `import type { ${reference} as ${remappedReference} } from '${importLocation}';`,
          );
        }
      });
    });
   ...
   */`,
      ...
      componentEvents.push(
        `  ${output.name}: EventEmitter<CustomEvent<${outputTypeRemapped.trim()}>>;`,
      );
