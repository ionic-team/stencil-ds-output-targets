...
const getFilteredComponents = (excludeComponents: string[] = [], cmps: ComponentCompilerMeta[]) =>
  sortBy<ComponentCompilerMeta>(cmps, (cmp: ComponentCompilerMeta) => cmp.tagName).filter(
...
export function generateProxies(config: Config, components: ComponentCompilerMeta[], outputTarget: OutputTargetSvelte) {
  ...
  const buildImports = (dir: string, ext = '') =>
